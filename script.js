...
function initializeVideoControls() {
    ...
    // Removed - replaced with double-tap system below
    ...
    function applySeek(targetTime, isSmartSeek = false) {
        ...
        // Faster debounce for smoother seeking

        const now = Date.now();

        const debounceTime = isSmartSeek ? 100 : 30;

        if (now - lastSeekTime < debounceTime && !isSmartSeek) {

            seekTimeout = setTimeout(() => applySeek(targetTime, isSmartSeek), debounceTime);
        ...
        // Only show loading for longer seeks to reduce visual flicker

        const seekDistance = Math.abs(targetTime - video.currentTime);
        ...
        // Enhanced seeking strategy for better performance

        if (typeof video.fastSeek === 'function' && seekDistance > 5) {
        ...
        // Optimized loading spinner management
        ...
        video.addEventListener('seeked', hideSpinner, { once: true });

        // Auto-hide spinner after 2 seconds max to prevent stuck loading
    ...
    function seekAndSync(targetTime, skipAnimation = false){

        const wasPlaying = !video.paused;

        const timeDiff = Math.abs(targetTime - video.currentTime);

        // For small seeks (< 3 seconds), use direct seeking to minimize lag

        if (timeDiff < 3 && wasPlaying) {

            applySeek(targetTime, true);

            if (!skipAnimation) showSeekFeedback(targetTime > video.currentTime ? '+' : '-', Math.round(timeDiff));

            return;

        }

        // For larger seeks, use the sync method but optimized

        const prevMuted = video.muted;

        // Only pause if necessary for very large seeks
        ...
        const tryResume = () => {
            ...
            // Lower threshold for faster resume
            ...
            video.removeEventListener('canplaythrough', tryResume);

            video.removeEventListener('seeked', tryResume);

            if (wasPlaying && timeDiff > 10) {
        ...
        if (timeDiff > 10) {
            ...
            video.addEventListener('canplaythrough', tryResume);

            video.addEventListener('seeked', tryResume);

            // Unmute when playback resumes
    ...
    // Visual feedback for seeks

    function showSeekFeedback(direction, seconds) {

        const feedback = document.createElement('div');

        feedback.className = 'seek-feedback';

        feedback.textContent = direction + seconds + 's';
    ...
    function handleKeyPress(e) {
        ...
        switch(e.key.toLowerCase()) {
            ...
            case 'arrowleft':
                ...
                seekAndSync(video.currentTime - 8);
    ...
    // Double-tap controls for YouTube-like experience
    ...
    function handleDoubleTap(e) {
        ...
        if (tapCount === 1) {
            ...
            const clickPosition = (clickX - rect.left) / videoWidth;

            

            if (clickPosition < 0.4) {

                // Left side - rewind 8 seconds

                seekAndSync(Math.max(0, video.currentTime - 8));

            } else if (clickPosition > 0.6) {

                // Right side - forward 8 seconds

                seekAndSync(Math.min(video.duration, video.currentTime + 8));

            } else {

                // Center - just toggle play/pause
    ...
    videoWrapper.addEventListener('click', handleDoubleTap);

    videoWrapper.addEventListener('touchend', handleDoubleTap);
    ...
    // Also on key interactions and touch events

    ['play', 'pause', 'seeking', 'volumechange', 'ratechange', 'touchstart'].forEach(evt => {
